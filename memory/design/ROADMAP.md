...
- **Design**: Complete. The `ViewModelTrait` includes optional `save_state` and `restore_state` methods. A two-version (`API Version` and `State Format Version`) scheme, automatically generated by `build.rs` scripts, ensures compatibility.
- **Implementation**: The `modules/loader` and `BindingRegistry` must be updated to check the `API Version`. The `restore_state` implementation in stateful modules must check the `State Format Version`.
