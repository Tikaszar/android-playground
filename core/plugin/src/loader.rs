...
use crate::{CreatePluginFn, Plugin};
...
impl PluginLoader {
    ...
    pub fn load_plugin(&mut self, path: &Path) -> Result<PluginId, PluginError> {
        if !path.exists() {
            return Err(PluginError::new("Plugin file does not exist"));
        }

        if path.extension() != Some(OsStr::new("so")) {
            return Err(PluginError::new("Plugin must be a .so file"));
        }

        unsafe {
            let library = libloading::Library::new(path)
                .map_err(|e| PluginError::new(&format!("Failed to load library: {}", e)))?;

            let create_fn: libloading::Symbol<CreatePluginFn> = library
                .get(b"create_plugin")
                .map_err(|e| PluginError::new(&format!("Failed to find create_plugin: {}", e)))?;
            ...
            if self.loaded_plugins.contains_key(&plugin_id) {
                return Err(PluginError::new("Plugin already loaded"));
            }
    ...
    pub fn unload_plugin(&mut self, id: &PluginId) -> Result<(), PluginError> {
        self.loaded_plugins
            .remove(id)
            .ok_or_else(|| PluginError::new("Plugin not found"))?;
        Ok(())
    }
