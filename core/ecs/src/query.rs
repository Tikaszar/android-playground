...
impl<T: Component> Query for WithoutComponent<T> {
    async fn execute(&self, _storages: &DashMap<ComponentId, Arc<dyn ComponentStorage>>) -> EcsResult<Vec<EntityId>> {
...
impl Query for EmptyQuery {
    ...
    async fn matches(&self, _entity: EntityId, _storages: &DashMap<ComponentId, Arc<dyn ComponentStorage>>) -> bool {
        true
    }
}
...
pub struct CachedQuery {
    inner: Box<dyn Query>,
    cache: Arc<parking_lot::RwLock<Option<Vec<EntityId>>>>,
}
