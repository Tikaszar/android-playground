    ...
    ($type:ty, $variant:ident, $atomic:ty) => {
        impl Atomic<$type> {
            ...
            pub fn load(&self) -> $type {
                match self {
                    Atomic::$variant(a) => a.load(Ordering::SeqCst),
                    _ => unreachable!(),
                }
            }
            ...
            pub fn store(&self, value: $type) {
                match self {
                    Atomic::$variant(a) => a.store(value, Ordering::SeqCst),
                    _ => unreachable!(),
                }
            }
            ...
            pub fn swap(&self, value: $type) -> $type {
                match self {
                    Atomic::$variant(a) => a.swap(value, Ordering::SeqCst),
                    _ => unreachable!(),
                }
            }
            ...
            pub fn compare_exchange(&self, current: $type, new: $type) -> Result<$type, $type> {
                match self {
                    Atomic::$variant(a) => a.compare_exchange(current, new, Ordering::SeqCst, Ordering::SeqCst),
                    _ => unreachable!(),
                }
            }
            ...
            pub fn fetch_add(&self, val: $type) -> $type {
                match self {
                    Atomic::$variant(a) => a.fetch_add(val, Ordering::SeqCst),
                    _ => unreachable!(),
                }
            }
            ...
            pub fn fetch_sub(&self, val: $type) -> $type {
                match self {
                    Atomic::$variant(a) => a.fetch_sub(val, Ordering::SeqCst),
                    _ => unreachable!(),
                }
            }
        }
...
impl Atomic<bool> {
    ...
    pub fn load(&self) -> bool {
        match self {
            Atomic::Bool(a) => a.load(Ordering::SeqCst),
            _ => unreachable!(),
        }
    }
    ...
    pub fn store(&self, value: bool) {
        match self {
            Atomic::Bool(a) => a.store(value, Ordering::SeqCst),
            _ => unreachable!(),
        }
    }
    ...
    pub fn swap(&self, value: bool) -> bool {
        match self {
            Atomic::Bool(a) => a.swap(value, Ordering::SeqCst),
            _ => unreachable!(),
        }
    }
    ...
    pub fn compare_exchange(&self, current: bool, new: bool) -> Result<bool, bool> {
        match self {
            Atomic::Bool(a) => a.compare_exchange(current, new, Ordering::SeqCst, Ordering::SeqCst),
            _ => unreachable!(),
        }
    }
}
...
impl Atomic<f32> {
    ...
    pub fn load(&self) -> f32 {
        match self {
            Atomic::F32(a) => f32::from_bits(a.load(Ordering::SeqCst)),
            _ => unreachable!(),
        }
    }
    ...
    pub fn store(&self, value: f32) {
        match self {
            Atomic::F32(a) => a.store(value.to_bits(), Ordering::SeqCst),
            _ => unreachable!(),
        }
    }
    ...
    pub fn swap(&self, value: f32) -> f32 {
        match self {
            Atomic::F32(a) => f32::from_bits(a.swap(value.to_bits(), Ordering::SeqCst)),
            _ => unreachable!(),
        }
    }
...
impl Atomic<f64> {
    ...
    pub fn load(&self) -> f64 {
        match self {
            Atomic::F64(a) => f64::from_bits(a.load(Ordering::SeqCst)),
            _ => unreachable!(),
        }
    }
    ...
    pub fn store(&self, value: f64) {
        match self {
            Atomic::F64(a) => a.store(value.to_bits(), Ordering::SeqCst),
            _ => unreachable!(),
        }
    }
    ...
    pub fn swap(&self, value: f64) -> f64 {
        match self {
            Atomic::F64(a) => f64::from_bits(a.swap(value.to_bits(), Ordering::SeqCst)),
            _ => unreachable!(),
        }
    }
